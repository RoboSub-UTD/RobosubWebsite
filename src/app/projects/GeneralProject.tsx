...
export default function GeneralProject({ProjectName, ProjectImage, AboutText, ImgArr}: {ProjectName: string, ProjectImage: string, AboutText: string, ImgArr: string[]}){
  return(
    <div>
      <ParallaxProvider>
        <Navbar/>
        <div className='GeneralProject'>
          ...
          <div className='projectAboutSection'>
            <Carousel className="Carousel" showArrows={true} emulateTouch={true} showThumbs={false}>
              {ImgArr.map((img: string | undefined, i: React.Key | null | undefined) => (<img draggable={false} className="carImage" src={img} key={i} alt="project"/>))}
            </Carousel>
